 ...
 * here breaks the feature with no error anywhere), the tilde expansion the
 * workspace's folder keys need before the URL can be built, and the
 * refusals — a token or path that would start reading as URL structure must
 * come back null, not half-escaped.
 */
...
describe('vscodeRemoteFolderUrl', () => {
  it('spells the Remote-SSH authority the way VS Code answers it', () => {
    expect(vscodeRemoteFolderUrl('hetzner', '/home/alexey/git/foo')).toBe(
      'vscode://vscode-remote/ssh-remote+hetzner/home/alexey/git/foo',
    );
  });
